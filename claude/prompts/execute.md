...
2. **Execute phase by phase** — work through the plan in phase order. After completing each phase, verify its outputs before moving to the next.
3. **Verify before claiming completion** — run tests, linting, and type checks. Confirm all acceptance criteria are met. Do not mark work as done until verification passes.
