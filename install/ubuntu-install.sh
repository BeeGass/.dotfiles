...
setup_kitty_local_conf() {
    section "[Ubuntu] Write kitty local.conf"
    local conf="$HOME/.config/kitty/local.conf"
    mkdir -p "$(dirname "$conf")"
    cat > "$conf" <<'EOF'
# Machine-local Kitty overrides (Linux)
# Auto-generated by ubuntu-install.sh -- edit freely

# Force X11 for picom compositing
