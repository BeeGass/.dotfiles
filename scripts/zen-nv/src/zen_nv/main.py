...
from textual.widgets import Footer, Static, DataTable, Label
...
def render_graph(datasets, width=50, height=15, theme_colors=None):
    ...
    for ds in datasets:
        ...
        plt.plot(
            list(ds["data"]), color=col, label=ds.get("label", ""), marker="braille"
        )
            ...
            datasets = [
                {
                    "data": history.cpu,
                    "label": "CPU",
                    "color": self.theme_config["cpu_color"],
                },
                {
                    "data": history.ram,
                    "label": "RAM",
                    "color": self.theme_config["ram_color"],
                },
            ]
        elif self.role == "gpu" and self.device_idx is not None:
            datasets = [
                {
                    "data": history.gpu_util.get(self.device_idx, []),
                    "label": "GPU",
                    "color": self.theme_config["gpu_color"],
                },
                {
                    "data": history.gpu_mem.get(self.device_idx, []),
                    "label": "VRAM",
                    "color": self.theme_config["mem_color"],
                },
            ]
    ...
    def refresh_table(self):
        ...
        if self.mode == "gpu":
            for device in self.devices:
                try:
                    ...
                    for p in procs:
                        ...
                        new_rows.append(
                            (pid_str, user_str, str(device.index), vram_str, cmd_str)
                        )
                except Exception as e:
            ...
            for p in psutil.process_iter(
                ["pid", "username", "cpu_percent", "memory_percent", "name", "cmdline"]
            ):
                try:
        ...
        with VerticalScroll(id="gpu-scroll"):
            for i, device in enumerate(self.devices):
                with Container(classes="device-row"):
                    yield StatsWidget(
                        role="gpu", device=device, theme_config=self.theme_config
                    )
                    yield GraphWidget(
                        role="gpu", device_idx=i, theme_config=self.theme_config
                    )
        ...
        with Container(id="proc-container"):
            ...
            with Vertical(classes="proc-box"):
                ...
                yield ProcessTableWidget(
                    mode="gpu", devices=self.devices, id="proc-gpu"
                )
