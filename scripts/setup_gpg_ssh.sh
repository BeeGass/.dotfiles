...
require_bins() {
  local miss=()
  for b in gpg gpgconf ssh awk sed; do have "$b" || miss+=("$b"); done
  if ((${#miss[@]})); then
...
import_from_github() {
  ...
  [[ -z "${GITHUB_GPG_URL}" ]] && {
    note "Skipping GitHub import (disabled)"
    return
  }
  [[ -f "$STAMP_FILE" ]] && {
    note "GitHub keys already imported (stamp)"
    return
  }
  if have curl; then
...
export_ssh_pubkey() {
  ...
  local fpr="$1"
  [[ -z "$fpr" ]] && {
    warn "No auth-capable subkey found; skipping export"
    return
  }
...
maybe_configure_git_signing() {
  ...
  [[ -z "$key" ]] && {
    warn "No key found to configure Git signing"
    return 0
  }
