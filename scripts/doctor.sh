...
[[ ":$PATH:" == *":$HOME/.local/bin:"* ]] || fail "~/.local/bin not in PATH"
pass "PATH ok"

# 2) Editor
command -v nvim >/dev/null || fail "neovim missing"
[ -f "$HOME/.config/nvim/init.vim" ] || fail "~/.config/nvim/init.vim symlink missing"
pass "Neovim wired to vimrc"
...
case "$(uname -s)" in
  ...
  Linux)
    if [[ -n "${TERMUX_VERSION-}" ]]; then
      [[ "$helper" == cache* ]] || echo "! credential.helper not 'cache' on Termux"
    else
      [[ "$helper" == libsecret* ]] || echo "! credential.helper not 'libsecret' on Ubuntu"
    fi
    ;;
esac
...
tmux -V >/dev/null || fail "tmux not runnable"
pass "tmux ok"
